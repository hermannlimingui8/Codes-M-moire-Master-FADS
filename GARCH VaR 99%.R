...
# ==============================================================================
# 🔁 BACKTEST (maintenant sûr)
# ==============================================================================
backtest_one <- function(name, VaR_vec, realized, alpha) {
  keep <- is.finite(VaR_vec) & is.finite(realized)
  VaR_vec <- VaR_vec[keep]; realized <- realized[keep]
  if (length(realized) < 50) return(data.frame(Méthode=name, Violations=NA, Taux_viol=NA, p_UC=NA, p_CC=NA))
  
  vt <- tryCatch(rugarch::VaRTest(alpha, actual=realized, VaR=VaR_vec), error=function(e) NULL)
  if (is.null(vt)) return(data.frame(Méthode=name, Violations=NA, Taux_viol=NA, p_UC=NA, p_CC=NA))
  
  data.frame(Méthode=name, Violations=sum(vt$actual.exceed), Taux_viol=sum(vt$actual.exceed)/length(realized),
             p_UC=if(!is.null(vt$uc.LRp)) round(vt$uc.LRp,4) else NA,
             p_CC=if(!is.null(vt$cc.LRp)) round(vt$cc.LRp,4) else NA)
}
...
# 4. Visualisation
df_long <- df_comp |>
  pivot_longer(cols = starts_with("VaR_"), names_to = "Méthode", values_to = "VaR") |>
  mutate(Méthode = case_match(
    Méthode,
    "VaR_GARCH" ~ "eGARCH-sstd",
    "VaR_Hist"  ~ "Historique",
    "VaR_Norm"  ~ "Normale",
    "VaR_RM"    ~ "RiskMetrics"
  ))

p <- ggplot(df_long, aes(x = date)) +
  geom_line(aes(y = realized), color = "gray60", linewidth = 0.5, alpha = 0.7) +
  geom_line(aes(y = VaR, color = Méthode), linewidth = 1) +
  labs(title = paste("Comparaison des VaR (α =", 100 * alpha_level, "%)"),
       subtitle = paste("Actif :", garch_obj$model_info$ticker),
       y = "Rendement log", x = "Date") +
